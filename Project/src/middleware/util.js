      ...
      if (data === null || data.length == 0) {
        ...
        bcrypt.genSalt(rounds, (err, salt) => {
          bcrypt.hash(password, salt, (err, hash) => {
            ...
            request.app
              .get("connection")
              .query(updatePassword, function (error, data) {
...
const filtersCondition = (req, status_name) => {
  ...
  const filter_date_range = req.body.filter_date_range;
  ...
  const subTickets = req.body.subTickets;
  var to_date = "";
  var from_date = "";
  ...
  if (from_date != "" && to_date == "") {
    ...
    search_query +=
      " AND date(creation_date) >= ? AND date(creation_date) <= ?";
    search_placeholders.push(from_date);
...
const paginationRule = (req, result) => {
  ...
  var total_count = 0;
  // console.log(result[1][0].total_count)
