...
const forgotButton = document.querySelector('#forgotPassswordForm button[type="submit"]');
...
if ( user_password !== undefined && user_password !== null )
{
    ...
    user_password.addEventListener('focus', (event) => {
        ...
        if(user_password.classList.contains('is-invalid') || password_span.classList.contains('password-span-error'))
        {
            password_span.classList.remove("password-span-error");
            user_password.classList.remove("is-invalid");
        }
...
if ( new_password !== undefined && new_password !== null )
{
    ...
    new_password.addEventListener('focus', (event) => {
        document.querySelector('#password_error').style.display = 'none';
        if(new_password.classList.contains('is-invalid') || password_span.classList.contains('password-span-error'))
        {
            password_span.classList.remove("password-span-error");
            new_password.classList.remove("is-invalid");
        }
...
if ( button !== undefined && button !== null )
{
    ...
    button.addEventListener('click', (event) => {
        ...
        client.post('/api/auth/login', JSON.stringify({'user_email' : user_email.value, 'user_password' : user_password.value}), (response, request) => {
...
} else if ( forgotButton !== undefined && forgotButton !== null )
{
    ...
    forgotButton.addEventListener('click', (event) => {
        ...
        client.post('/api/user/password-reset', JSON.stringify({'email' : user_email.value, 'password' : new_password.value}), (response, request) => {
            response = JSON.parse(response);
            if (request.status != 200)
            {
                showErrorMessage(response);
...
} else if ( signupButton !== undefined && signupButton !== null)
{
    ...
    [email, first_name, last_name, password].forEach((element) => {
        element.addEventListener('focus', (event) => {
            if(element.classList.contains('is-invalid') || element.classList.contains('password-span-error'))
            {
    ...
    signupButton.addEventListener('click', (event) => {
        ...
        [email, first_name, last_name, password].forEach((element) => {
            if(element != undefined && element.value == '')
            {
                ...
                {
                    document.querySelector('#password_error').classList.add("password-span-error");
                }
            }
        });
        ...
        client.post('/api/user/create', JSON.stringify({'email' : user_email.value, 'password' : new_password.value}), (response, request) => {
            response = JSON.parse(response);
            if (request.status != 200)
            {
                showErrorMessage(response);
...
//     client.post('/api/user/verify', JSON.stringify({'user_password' : user_password.value}), (response, request) => {
//         response = JSON.parse(response);
//         if (request.status != 200)
//         {
//             showErrorMessage(response);
